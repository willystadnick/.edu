...
// luz do sol
light_source{               // luz externa dar impressão
...
plane{ <0,1,0>,-1    // chão com textura de madeira escura
      texture{
        T_Wood16
        pigment{}
           finish{reflection 0.1    // dar um brilho no chão         
                 }
             }
     }
...
object{Cube scale <6,6,0.3>   // parede de fundo de cor
    ...
    rotate<0,0,0>
    translate<2,0,11>        // localização da parede
} 
...
translate<0,0,0>
  ...
  rotate<0,0,-20>
  translate<-1.15,-1.15,2>// inclinação para dar impressão de janela aberta
  }
  
//*******************************************************            
// portas
#declare porta = union{          // declaração de portas do armario
...
#declare gaveta = union{       // declaração das gavetas
    superellipsoid {<0.05,0.5>
          scale<0.4,0.18,0.05>
          texture{T_Wood3
          pigment{}
...
union{
  difference{                 // torneira da pia
      torus {0.2,0.01
       ...
       translate<-0.13,-0.43,0>// localização datorneira
  }
...
difference{                          // funil da pia 
    ...
    difference{
      ...
      superellipsoid {<0.1,0.1> // parte de inox para criação das bordas
        scale<0.7,0.05,1.21>
        texture{T_Silver_5A
        pigment{}
        finish{reflection 0.01}
     ...
     object {porta        // porta do balcão da pia
            rotate<0,-90,0>
     ...
     object {gaveta     // gaveta do balcão da pia
            rotate<0,-90,0>
...
union
 ...
 cylinder
  {
  <0.05,0,0>, <0,0,0>, 0.065 //botão da boca do fogão
  texture
   {
   T_Chrome_1A
   pigment{}
 ...
 cylinder
  {
  <0.05,0,0>, <0,0,0>, 0.065 // botão do forno do fogão
  texture
   {
   T_Chrome_1A
   pigment{}
  ...
  translate <4.52,0.45,8.5>
    ...
    disc {<0, 0, 0>z,0.11, 0.05  //botão das bocas do fogão
    translate<3.52,0.45,8.5>
    }
    disc {<0, 0, 0>z,0.11, 0.05  //botão das bocas do fogão
    translate<3.77,0.45,8.5>
    ...
    superellipsoid {<0.01,0.03> // parte  superior de inox do fogão
      scale<0.72,0.03,0.5>
       texture{T_Silver_5A
       pigment{}
     ...
     object{Cube scale <0.62,0.4,0.5>
       ...
       cylinder {<0.2,0,0>,<0,0,0>,0.175 // bocas do fogão
          texture{T_Chrome_2C  
              pigment{}
              }
          rotate<90,0,90>
...
//tampa do forno
    object{Cube scale <0.68,0.47,0.005> // tampa d vidro do forno do fogão
          texture{
                 T_Glass1
                 pigment{}
                 finish{reflection 0.1}
...
// tampa fogão
    object{Cube scale <0.71,0.55,0.02> //tampa superior de vidro do fogão
          texture{
                 T_Glass1
                 pigment{}
                 finish{reflection 0.1}
...
union{       // parte inferior do balcão lado esquerdo e direito da geladeira
  difference{
    ...
    object{Cube scale <0.92,0.9,0.7>// parte a onde vai a geladeira espaço aberto
          texture{
               T_Wood3
                 pigment{}
           }
...
translate<-5.8,0,7.1>
     ...
     object {porta   //porta do balcão inferior lado esquerdo da geladeira
            rotate<0,0,0>
            translate<-1.25,-0.2,9.35>
            }
     object {gaveta //gaveta do balcão inferior lado esquerdo da geladeira
            rotate<0,0,0>
     ...
     object {porta   //porta do balcão inferior lado direito da geladeira
            scale<1.3,0,0>
            rotate<0,0,0>
...
union{                                   //parte superior portagarrafas
  ...
  object{Cube scale <0.45,0.7,0.25>
    ...
    object{Cube scale <0.16,0.2,0.3>     // burraco onde vai as garrafas só faltou as SKOL
          texture{
               T_Wood3
                 pigment{}
           }
...
union{                          // parte superior ao fogão com duas portas pequenas
...
//portas
  union{ 
    superellipsoid {<0.05,0.5>   // portas pequenas da parte superior do fogão
...
rotate <0,0,0>
translate<0,3.66,2>
}                //fim das portas da parte  superior do balcão  sobre do fogão
...
# declare  perna = union{      // declaração das pernas da mesa
...
#declare cadeira = union{    // declaração de cadeira
...
union{               // encosto das cadeiras
  object{pernacad    // não é as pernas foi usado o mesmo codigo  
...
} // final da declaração das cadeiras
